...
function loadPre() {
	...
	/*
	var div = '<div style="width:830px;height: 600px;overflow: hidden;margin: auto;bgcolor: #F1F1F1;">' +
		'<iframe height="100%" width="100%" id="PreIFrame" src="http://teamspace.pg.com/sites/cppdpolicy/WPPages/Administration/Presentations.aspx"   frameborder="0"></iframe>' +
		'</div>';
...
function loadGuidelines() {
	...
	var div = '<div id="divGoBack" style="display:none;padding-bottom:10px" align="right"><a href="javascript:GoBackHomeUpload()">Go back</a></div>' +
		...
		//'<iframe id="FrameSet" src="http://teamspace.pg.com/sites/cppdpolicy/Lists/GuidelinesPrinciples/AllItems.aspx"   frameborder="0"></iframe>' +
		'<iframe id="FrameSet"'+
...
function CreateHomeLinksTitle() {
	document.getElementById('headersec').innerText = 'Home Links';
	var div = '<div id="divGoBack" style="display:none;padding-bottom:10px" align="right"><a href="javascript:GoBackHomeUpload()">Go back</a></div>' +
		'<div id="DivIframe" style="width: 805px;height: 400px;overflow: hidden;margin: auto;bgcolor: #F1F1F1;">' +
		
		'<iframe id="FrameSet"'+
'style="width: 990px; height: 1000px; margin-left: -155px;margin-top: -150px; margin-right: 0px; border: 0 solid; bgcolor: #F1F1F1; overflow-x: hidden"'+
'src="http://teamspace.pg.com/sites/cppdpolicy/Lists/HomeLinksTitle/AllItems.aspx"   frameborder="0"></iframe>' +
		'</div>';
	document.getElementById("tdIframe").innerHTML = div;
}
...
function getUserName() {
	document.getElementById('ctl00_aspLoginName').style.display = 'none';
	var Uname = document.getElementById('ctl00_aspLoginName').innerHTML;
	//alert(Uname);
	try {
		//debugger
		var oWs = spUserInformation.getGroupCollectionFromUser(Uname);
		if (oWs.status == 200) {
			//alert(oWs.responseText);
			var groups = oWs.responseXML.getElementsByTagName('Group');
			if (groups.length > 0) {
				for (var i = 0; i < groups.length; i++) {
					//debugger
					var grpID = groups[i].getAttribute('ID');
					if (grpID == 16) {
						IsAdmin = true;
					} else if (grpID == 35) {
						IsSuperAdmin = true;
				...
				if (!IsSuperAdmin) {
					if (document.getElementById('divSuperAdmin'))
						document.getElementById('divSuperAdmin').style.display = 'none';
				}
			}
